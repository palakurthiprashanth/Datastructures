...
```
The sort() function in javascript compares 
the values as strings in lexicographical order.
 Due to this reason, 50 will be considered 
 greater than the 300 since 5 is 
 greater than 3. Thus, sort() function 
 will produce unexpected output.
  We can resolve this issue by using a 
  function to compare two values in the
   argument of the sort() function. 
   In this way, arrays values will 
   be sorted according to the value 
   returned from the compare function.
```
```
Time complexity
